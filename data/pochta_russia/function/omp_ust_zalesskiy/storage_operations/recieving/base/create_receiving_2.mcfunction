data modify storage pochta_russia:omp_ust_zalesskiy Recieving.2 set from entity @s Items

#Запускаем заполнение отправителя от ближайшей к автору команды (вагонетка) рамки с получателем 
execute as @e[type=glow_item_frame,tag=pochta_sender,nbt={Item:{id:"minecraft:writable_book"}},sort=nearest,limit=1] at @s run function pochta_russia:omp_ust_zalesskiy/storage_operations/senders/base/set_2
