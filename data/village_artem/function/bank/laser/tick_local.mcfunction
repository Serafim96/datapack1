# Если ядерный реактор выключен, то открываем все и выходим
execute if data storage village_artem {nuclear_reactor:{powered:0}} run return run function village_artem:bank/laser/reactor_disabled
...
execute if entity @a[tag=village_artem_bank_client,x=-5083,dx=3,y=38,dy=3,z=8565,dz=8,nbt={SelectedItem:{components:{"minecraft:custom_data":{Tags:["village_bank_card"]}}}}] if data storage minecraft:village_artem {BankLaserProcessing:0} if data storage minecraft:village_artem {BankLaserOpened:0} run function village_artem:bank/laser/api/open
execute if entity @a[tag=village_artem_bank_client,x=-5083,dx=3,y=38,dy=3,z=8565,dz=8,nbt={equipment:{offhand:{components:{"minecraft:custom_data":{Tags:["village_bank_card"]}}}}}] if data storage minecraft:village_artem {BankLaserProcessing:0} if data storage minecraft:village_artem {BankLaserOpened:0} run function village_artem:bank/laser/api/open
