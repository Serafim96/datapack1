...
# [WAS] execute as @e[scores={AngmarTombTimeBuffOn=1..},gamemode=!spectator] at @s run function angmar_tomb:time_buff/buff_result/head_particles
# [AI: H4 — селектор содержит gamemode= ⇒ нацелен только на игроков; добавлен type=player (тип стал явным, семантика прежняя)]
execute as @e[type=player,scores={AngmarTombTimeBuffOn=1..},gamemode=!spectator] at @s run function angmar_tomb:time_buff/buff_result/head_particles
